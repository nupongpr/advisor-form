...
export default async function AdminDashboard() {
  ...
  const langLabel = (v: string) => (v === "en" ? "Eng" : "ไทย");
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted">
                ...
                <TableHead className="font-semibold text-foreground">ภาษา</TableHead>
            ...
            <TableBody>
              {responses.map((r, i) => (
                <TableRow key={r.id} className="border-0 even:bg-muted/40 hover:bg-accent/40">
                  ...
                  <TableCell className="text-sm text-muted-foreground">{langLabel(r.language)}</TableCell>
              ...
              {total === 0 && <TableRow className="border-0"><TableCell colSpan={6} className="text-center text-muted-foreground">ยังไม่มีคำตอบ</TableCell></TableRow>}
            </TableBody>
          </Table>
        </CardContent>
      </Card>
